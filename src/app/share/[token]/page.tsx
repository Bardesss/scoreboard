    ...
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: '#f5f0e8' }}>
      ...
      <div className="w-full max-w-sm rounded-3xl p-6 shadow-lg" style={{ background: '#fefcf8', border: '1px solid #e8e1d8' }}>
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <p className="font-headline font-black text-base" style={{ color: '#1e1a14' }}>{pg.league.name}</p>
        ...
        <ul className="space-y-2">
          {pg.scores.map((s, i) => (
            <li key={s.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl" style={{ background: i === 0 ? 'rgba(245,166,35,0.08)' : '#f5f0e8' }}>
              <span className="font-headline font-black text-sm w-5" style={{ color: i === 0 ? '#f5a623' : '#c5b89f' }}>#{i + 1}</span>
              <span className="flex-1 font-headline font-semibold text-sm" style={{ color: '#1e1a14' }}>{s.player.name}</span>
        ...
        <p className="mt-6 text-center text-xs font-body" style={{ color: '#c5b89f' }}>Logged with Dice Vault</p>
      </div>
    </div>
  )
}
